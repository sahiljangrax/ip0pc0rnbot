        ...
        try: 
         ...
         #       newPoint = await db.get_point(refUserId)
             if AUTH_CHANNEL and await is_req_subscribed(client, message):
                        ...
                        reply_markup = InlineKeyboardMarkup(buttons)
                        m=await message.reply_sticker("CAACAgUAAxkBAAELVLllwnoXcNEolJLj0HXbbDRTqBJrtwACzwkAApdT2VcpjUvvR4HhRzQE") 
...
async def save_tutorial(client, message):
    ...
    try:
        ...
        return await message.reply_text("<b>Command Incomplete!!\n\nuse like this -</b>\n\n<code>/set_caption https://t.me/LmOwnerBot</code>")    
...
async def set_shortner(c, m):
    ...
    try:
        URL = m.command[1]
        API = m.command[2]
        resp = requests.get(f'https://{URL}/api?api={API}&url=https://telegram.dog/LmOwnerBot').json()
